...
async def serve(config: ServerConfig) -> None:
    ...
    print(f"Earch 2064 TCP listening on {sockets}")
    if http_server:
        host, port = http_server.server_address
        print(f"Earch 2064 web status on http://{host}:{port}")
    try:
        async with server:
...
def parse_args(argv: list[str] | None = None) -> ServerConfig:
    parser = argparse.ArgumentParser(description="Run the Earch 2064 server.")
    parser.add_argument("--db", default="var/earch2064.sqlite")
